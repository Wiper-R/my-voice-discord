...
class Dispatcher(commands.Cog):
    ...
    async def on_channel_join(self, member: discord.Member, state: discord.VoiceState):
        guild_config = await self.voice_cog.fetch_guild_config(member.guild)
        ...
        config = await VoiceConfig.get_or_none(channel_id=state.channel.id)
    ...
    async def on_channel_left(self, member: discord.Member, state: discord.VoiceState):
        ...
        channel = await VoiceChannels.get_or_none(id=state.channel.id)
    ...
    async def update_sequential_channels(self, channel_id):
        config = await VoiceConfig.get_or_none(channel_id=channel_id)
